...
export default {
  ...
  methods: {
    ...
    estimate: function() {
      ...
      var instance = this;
      axios
        .post("api/predict/logisticRegression", {
          trainingSet: {
        ...
        .then(response => {
          ...
          var resultStrings = data.map((element, index) => {
            var name = instance.targetNameList[index];
            // 小数点以下2桁まで切り落とす。
            var percentage = Math.round(element*10000)/100;
            return `${name}の確立：${percentage}%`;
          });
          instance.isLoading = false;
          instance.$refs.resultDialog.show(resultStrings, []);
        })
        .catch(err => {
          console.error(err);
          instance.isLoading = false;
